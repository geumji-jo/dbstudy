...
CREATE SEQUENCE DEPARTMENT_SEQ
    ...
    NOMAXVALUE     -- 번호에 상한선이 없다.(생략 가능)  MAXVALUE 100 : 번호를 100까지만 생성한다.
    NOMINVALUE     -- 번호에 하한선이 없다.(생략 가능)  MINVALUE 100 : 번호의 최소값이 100이다.
    NOCYCLE        -- 번호 순환이 없다.(생략 가능)      CYCLE : 번호가 MAXVALUE에 도달하면 다음 번호는 MINVALUE이다.
    NOCACHE        -- 메모리 캐시를 사용하지 않는다.    CACHE : 메모리 캐시를 사용한다.(사용 않는 것이 좋다.)  
    ORDER          -- 번호 건너뛰기가 없다.            NOORDER : 번호 건너뛰기가 가능하다.
;    

-- 시퀀스에서 번호 뽑는 함수 : NEXTVAL
-- SELECT DEPARTMENT_SEQ.NEXTVAL FROM DUAL; -- 오라클에서는 테이블에 없는 데이터를 조회하려면 DUAL 테이블을 사용한다.(오라클이 만들어둔 DUAL 테이블)
...
--2. 부서번호(DEPART)가 1인 부서의 근무하는 사원들의 급여를 500000원 증가시키시오.
UPDATE EMPLOYEE_TBL
...
--2. 지역(LOCATION)이 '서울'인 부서를 삭제하시오. ('서울'에서 근무하는 사원의 부서번호(DEPART)가 ON DELETE SET NULL 외리캐 옵션에 의해서 NULL로 처리된다,)
